function [totalStats] = testAllACBRCombinations(dataset,isWeighted,FeatureSelection)
    ...
    elseif isequal(dataset(1:8),'WAVEFORM')
        [STDData, pf] = prepare_waveform_kmeans('datasets_iml_course/waveform/waveform.arff');
        Categories = pf.class.values()';
    end    
        ...
        for K=[3,5,7]
            ...
            for i = 1:NFolds
                ...
                for j=1:size(TestData,1)
                    ...
                    [newStats, weightedSTDData, weightedCategories, weightedGoodness, weightedClass,Retained, numOblived] = callACBRAlgorithm(isWeighted,getCombination(c),Stats,weightedSTDData,weightedCategories,weightedGoodness,initGoodness, instance,instanceClass,K,Weights, Features);
                ...
                numRetainedV = [numRetainedV, ret];
                numOblivedV = [numOblivedV, obl];
            ...
            numRetainedVK = [numRetainedVK; numRetainedV];
            numOblivedVK = [numOblivedVK; numOblivedV];
            
        end
        combStats = struct('numValid',numValidVK,'total',totalVK,'dataSetSizeChange',sizeChangeVK,'efficiency',efficiencyVK,'retained',numRetainedVK,'oblived',numOblivedVK);      
